                       ...
                       uint8_t arm_rd, uint8_t arm_rm)
{
    if (insn->has_modrm && insn->mod == 3) {
        ...
        if (imm <= 0xFFFF && imm >= 0) {
            emit_add_reg_imm32(code_buf, arm_rd, (u32)imm);
        } else {
            /* Large immediate: load into temp register then add */
            uint8_t tmp = 16;
            emit_mov_reg_imm64(code_buf, tmp, imm);
            emit_add_reg_reg(code_buf, arm_rd, tmp);
        }
    } else {
                       ...
                       uint8_t arm_rd, uint8_t arm_rm)
{
    if (insn->has_modrm && insn->mod == 3) {
        ...
        u64 imm = (u64)insn->imm;
        if (imm <= 0xFFFF && imm >= 0) {
            emit_sub_reg_imm32(code_buf, arm_rd, (u32)imm);
        } else {
            /* Large immediate: load into temp register then subtract */
            uint8_t tmp = 16;
            emit_mov_reg_imm64(code_buf, tmp, imm);
            emit_sub_reg_reg(code_buf, arm_rd, tmp);
        }
    } else {
        /* Memory operand */
        emit_sub_reg_reg(code_buf, arm_rd, arm_rm);
                       ...
                       uint8_t arm_rd, uint8_t arm_rm)
{
    if (insn->has_modrm && insn->mod == 3) {
        ...
        u64 imm = (u64)insn->imm;
        if (imm <= 0xFFFF && imm >= 0) {
            emit_and_reg_imm32(code_buf, arm_rd, (u32)imm);
        } else {
            /* Large immediate: load into temp register then AND */
            uint8_t tmp = 16;
            emit_mov_reg_imm64(code_buf, tmp, imm);
            emit_and_reg_reg(code_buf, arm_rd, tmp);
        }
    } else {
        emit_and_reg_reg(code_buf, arm_rd, arm_rm);
    }
                      ...
                      uint8_t arm_rd, uint8_t arm_rm)
{
    if (insn->has_modrm && insn->mod == 3) {
        ...
        uint8_t tmp = 16;
        emit_mov_reg_imm64(code_buf, tmp, imm);
        emit_orr_reg_reg(code_buf, arm_rd, tmp);
    } else {
        emit_orr_reg_reg(code_buf, arm_rd, arm_rm);
    }
}
                       ...
                       uint8_t arm_rd, uint8_t arm_rm)
{
    ...
    if (insn->has_modrm && insn->mod == 3 && insn->reg == insn->rm) {
        emit_mov_reg_imm64(code_buf, arm_rd, 0);
        return;
    }

    if (insn->has_modrm && insn->mod == 3) {
        /* Register XOR: r64, r/m64 */
        emit_xor_reg_reg(code_buf, arm_rd, arm_rm);
    } else if (insn->imm_size > 0) {
        /* Immediate XOR - load into temp then XOR */
        u64 imm = (u64)insn->imm;
        uint8_t tmp = 16;
        emit_mov_reg_imm64(code_buf, tmp, imm);
        emit_xor_reg_reg(code_buf, arm_rd, tmp);
    } else {
        emit_xor_reg_reg(code_buf, arm_rd, arm_rm);
    }
}
                       ...
                       uint8_t arm_rd, uint8_t arm_rm)
{
     ...
     */
    emit_mvn_reg_reg(code_buf, arm_rd, arm_rm);
}
