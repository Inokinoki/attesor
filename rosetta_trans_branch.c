...
int translate_bcond(ThreadState *state, const uint8_t *insn)
{
    ...
    uint8_t N = (state->host.pstate >> 31) & 1;  /* Negative */
    uint8_t Z = (state->host.pstate >> 30) & 1;  /* Zero */
    uint8_t C = (state->host.pstate >> 29) & 1;  /* Carry */
    uint8_t V = (state->host.pstate >> 28) & 1;  /* Overflow */
...
int translate_tbz(ThreadState *state, const uint8_t *insn)
{
    ...
    if ((state->host.x[rt] & (1ULL << bit)) == 0) {
        state->host.pc += imm14 * 4;
    }
