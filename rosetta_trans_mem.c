...
int translate_ldr(ThreadState *state, const uint8_t *insn) HOT_PATH
{
    ...
    switch (size) {
        case 0:
            state->host.x[rt] = *(uint8_t *)host_addr;
            break;
        case 1:
            state->host.x[rt] = *(uint16_t *)host_addr;
            break;
        case 2:
            state->host.x[rt] = *(uint32_t *)host_addr;
            break;
        case 3:
            state->host.x[rt] = *(uint64_t *)host_addr;
            break;
    }
...
int translate_str(ThreadState *state, const uint8_t *insn)
{
    ...
    switch (size) {
        case 0:
            *(uint8_t *)host_addr = (uint8_t)state->host.x[rt];
            break;
        case 1:
            *(uint16_t *)host_addr = (uint16_t)state->host.x[rt];
            break;
        case 2:
            *(uint32_t *)host_addr = (uint32_t)state->host.x[rt];
            break;
        case 3:
            *(uint64_t *)host_addr = state->host.x[rt];
            break;
    }
...
int translate_ldp(ThreadState *state, const uint8_t *insn)
{
    ...
    uint64_t addr = state->host.x[rn] + (imm7 << (size + 2));
    ...
    switch (size) {
        case 2:
            state->host.x[rt] = *(uint32_t *)host_addr;
            state->host.x[rt2] = *(uint32_t *)((uint64_t)host_addr + 4);
            break;
        case 3:
            state->host.x[rt] = *(uint64_t *)host_addr;
            state->host.x[rt2] = *(uint64_t *)((uint64_t)host_addr + 8);
            break;
    }
...
int translate_stp(ThreadState *state, const uint8_t *insn)
{
    ...
    switch (size) {
        case 2:
            *(uint32_t *)host_addr = (uint32_t)state->host.x[rt];
            *(uint32_t *)((uint64_t)host_addr + 4) = (uint32_t)state->host.x[rt2];
            break;
        case 3:
            *(uint64_t *)host_addr = state->host.x[rt];
            *(uint64_t *)((uint64_t)host_addr + 8) = state->host.x[rt2];
            break;
    }
...
int translate_ldrh(ThreadState *state, const uint8_t *insn)
{
    ...
    uint64_t addr = state->host.x[rn] + (imm12 << 1);
...
int translate_ldrsb(ThreadState *state, const uint8_t *insn)
{
    ...
    state->host.x[rt] = *(int8_t *)host_addr;  /* Sign-extended */
...
int translate_ldrsh(ThreadState *state, const uint8_t *insn)
{
    ...
    state->host.x[rt] = *(int16_t *)host_addr;  /* Sign-extended */
...
int translate_ldrsw(ThreadState *state, const uint8_t *insn)
{
    ...
    uint64_t addr = state->host.x[rn] + (imm12 << 2);
    ...
    state->host.x[rt] = *(int32_t *)host_addr;  /* Sign-extended */
...
int translate_ldr_reg(ThreadState *state, const uint8_t *insn)
{
    ...
    uint64_t addr = state->host.x[rn] + state->host.x[rm];
