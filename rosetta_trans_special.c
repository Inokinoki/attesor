...
int translate_cpuid(ThreadState *state, const uint8_t *insn)
{
    ...
    uint32_t leaf = (uint32_t)state->host.x[0];  /* EAX */
    uint32_t subleaf = (uint32_t)state->host.x[2];  /* ECX */
    ...
    state->host.x[0] = eax;
    state->host.x[3] = ebx;  /* RBX */
    state->host.x[1] = ecx;  /* RCX */
    state->host.x[2] = edx;  /* RDX */
...
int translate_rdtsc(ThreadState *state, const uint8_t *insn)
{
    ...
    state->host.x[0] = tsc & 0xFFFFFFFF;  /* EAX */
    state->host.x[2] = tsc >> 32;  /* EDX */
...
int translate_shld(ThreadState *state, const uint8_t *insn)
{
    ...
    uint64_t dst = state->host.x[rd];
    uint64_t src = state->host.x[rn];
...
int translate_cqo(ThreadState *state, const uint8_t *insn)
{
    ...
    int64_t rax = (int64_t)state->host.x[0];

    if (rax < 0) {
        state->host.x[2] = 0xFFFFFFFFFFFFFFFFULL;  /* RDX = -1 */
    } else {
        state->host.x[2] = 0;
    }
...
int translate_cwd(ThreadState *state, const uint8_t *insn)
{
    ...
    int16_t ax = (int16_t)(state->host.x[0] & 0xFFFF);

    if (ax < 0) {
        state->host.x[2] = 0xFFFF;  /* DX = 0xFFFF */
    } else {
        state->host.x[2] = 0;
    }
...
int translate_cdq(ThreadState *state, const uint8_t *insn)
{
    ...
    int32_t eax = (int32_t)(state->host.x[0] & 0xFFFFFFFF);

    if (eax < 0) {
        state->host.x[2] = 0xFFFFFFFF;  /* EDX = 0xFFFFFFFF */
    } else {
        state->host.x[2] = 0;
    }
