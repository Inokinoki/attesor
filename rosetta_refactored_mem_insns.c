...
void translate_ldrb(uint32_t encoding, ThreadState *state)
{
    ...
    state->host.x[rt] = value;  /* Zero-extend */
}
...
void translate_ldrsw(uint32_t encoding, ThreadState *state)
{
    ...
    state->host.x[rt] = (uint64_t)value;  /* Sign-extend */
}
...
void translate_strb(uint32_t encoding, ThreadState *state)
{
    uint8_t rt = encoding & 0x1F;
    uint64_t addr = get_mem_address(encoding, state);
    uint8_t value = (uint8_t)state->host.x[rt];
    *(uint8_t *)addr = value;
}
...
void translate_strh(uint32_t encoding, ThreadState *state)
{
    uint8_t rt = encoding & 0x1F;
    uint64_t addr = get_mem_address(encoding, state);
    uint16_t value = (uint16_t)state->host.x[rt];
    *(uint16_t *)addr = value;
}
...
void translate_ldp(uint32_t encoding, ThreadState *state)
{
    ...
    uint64_t addr = state->host.x[rn] + offset;

    if (sf) {
        state->host.x[rt1] = *(uint64_t *)addr;
        state->host.x[rt2] = *(uint64_t *)(addr + 8);
    } else {
        state->host.x[rt1] = *(uint32_t *)addr;
        state->host.x[rt2] = *(uint32_t *)(addr + 4);
    }
}
...
void translate_stp(uint32_t encoding, ThreadState *state)
{
    ...
    if (sf) {
        *(uint64_t *)addr = state->host.x[rt1];
        *(uint64_t *)(addr + 8) = state->host.x[rt2];
    } else {
        *(uint32_t *)addr = state->host.x[rt1];
        *(uint32_t *)(addr + 4) = state->host.x[rt2];
    }
}
...
void translate_ldur(uint32_t encoding, ThreadState *state)
{
    ...
    uint64_t addr = state->host.x[rn] + imm9;

    switch (size) {
        case 2:
            state->host.x[rt] = *(uint32_t *)addr;
            break;
        case 3:
            state->host.x[rt] = *(uint64_t *)addr;
            break;
    }
}
